...
import { useGegeMyPerformanceStore } from '../../store'
...
const { performance, isLoading, error, refetch } = useGegeMyPerformanceStore(selectedYear)
...
const activeYear = computed(() => {
  return selectedYear.value
    ?? performance.value?.history[0]?.year
    ?? performance.value?.availableYears[0]
    ?? null
})

const profile = computed(() => ({
  name: performance.value?.agent.name ?? '我的业绩',
  agentCode: performance.value?.agent.agentCode ?? '',
  designationName: formatDesignation(performance.value?.agent.designationName),
  periodLabel: formatPeriod(performance.value?.period),
}))

const currentMetrics = computed(() => performance.value?.current)
...
const history = computed(() => performance.value?.history ?? [])
const availableYears = computed(() => performance.value?.availableYears ?? [])
const hasHistory = computed(() => availableYears.value.length > 0)
...
<template>
  <view class="min-h-screen bg-background px-4 pb-16 pt-4">
    ...
    <view v-if="error && !performance" class="card mt-4 p-4">
      <t-empty icon="error-circle" :description="error.message || '数据加载失败'" />
    </view>

    <view v-else-if="!hasHistory && !isLoading" class="card mt-4 p-4">
